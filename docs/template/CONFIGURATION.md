...
|-------------|---------------|---------|
| `attested-delivery/rust-template` | `your-org/your-repo` (full path) | `acme/my-cli` |
| `attested-delivery/rust_template` | `your-org/your_crate` (full path, underscored) | `acme/my_cli` |
| `attested-delivery` | Your GitHub org or username | `acme` |
...
find . -type f \
  ...
  ! -name 'Cargo.lock' \
  -exec sed -i '' \
    -e "s|attested-delivery/rust-template|${OWNER}/${REPO}|g" \
    -e "s|attested-delivery/rust_template|${OWNER}/${CRATE}|g" \
    -e "s|attested-delivery|${OWNER}|g" \
...
| `repository` | `https://github.com/attested-delivery/rust-template` | Auto-replaced by `template-init` |
| `homepage` | `https://github.com/attested-delivery/rust-template` | Auto-replaced by `template-init` |
...
Instructions for [GitHub Copilot coding agent](https://docs.github.com/en/copilot/using-github-copilot/using-copilot-coding-agent) and [Claude Code](https://docs.anthropic.com/en/docs/claude-code). Located at the repo root.
...
- Read by the Copilot coding agent when it works on issues and PRs, and by Claude Code
...
- Edit `AGENTS.md` at the repo root to update project-wide conventions.
